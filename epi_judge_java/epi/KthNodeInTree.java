...
public class KthNodeInTree {
	...
	return findKthNodeHelper(tree, k);
    }

    public static BinaryTreeNode<Integer> findKthNodeHelper(BinaryTreeNode<Integer> tree, int k) {

	int leftSize = tree.left != null ? tree.left.size : 0;

	if (k > leftSize + 1) {
	    return findKthNodeHelper(tree.right, k - (leftSize + 1));
	} else if (k == leftSize + 1) {
	    return tree;
	} else {
	    return findKthNodeHelper(tree.left, k);
	}
